...
#    if (bolt.height-bolt.width//2) is
...
class Alien(GImage):
    ...
    """
    def collides(self, bolt):
        if bolt.isPlayerBolt() and self.contains((bolt.x,bolt.y)):
            return True
        return False
    # ADD MORE METHODS (PROPERLY SPECIFIED) AS NECESSARY
...
class Bolt(GRectangle):
    ...
    def __init__(self, x, y, width, height, fillcolor, linewidth, linecolor, pos):
        ...
        super().__init__(x=x, y=y, width=width, height=height, fillcolor=fillcolor, linewidth=linewidth, linecolor=linecolor)
        if pos == True:
    ...
    def isPlayerBolt(self):
        """
        Returns True if bolt is a player bolt, false otherwise
        """
        if self._velocity > 0:
            return True
        else:
