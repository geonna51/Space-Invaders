...
class Ship(GImage):
    ...
    def collides(self,bolt):
        ...
        """
        assert isinstance(bolt, GRectangle)
        top_left = (bolt.x-bolt.width//2, bolt.y+bolt.height//2)
        bot_left = (bolt.x-bolt.width//2, bolt.y-bolt.height//2)
        top_rght = (bolt.x+bolt.width//2, bolt.y+bolt.height//2)
        bot_rght = (bolt.x+bolt.width//2, bolt.y-bolt.height//2)
        if self.contains(top_left) or self.contains(bot_left) or \
        self.contains(top_rght) or self.contains(bot_rght):
...
class Bolt(GRectangle):
    ...
    def __init__(self, x, y, width, height, fillcolor, linewidth, linecolor, pos):
        """
        ...
        fillcolor=fillcolor, linewidth=linewidth, linecolor=linecolor)
        if pos == True:
