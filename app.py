...
class Invaders(GameApp):
    ...
    def update(self,dt):
        ...
        if self.input.is_key_pressed('s') and self._state == STATE_INACTIVE:
            ...
            self._wave = Wave(width = self.width, height = self.height)
            self._state = STATE_ACTIVE
        elif self._state == STATE_ACTIVE:
            self._text = None
            self._wave.update(self.input, dt)
            if self._wave.checkChange():
                self._state = STATE_PAUSED
            if self._wave.getLives() == 0 or self._wave.getLost():
                self._state = STATE_COMPLETE
                self._text = GLabel(text="Game Over, You Lost", font_size=64, \
                x=self.width//2, y=self.height//2)
            if self._wave.getWon():
                self._state = STATE_COMPLETE
                self._text = GLabel(text="Game Over, You Won!", font_size=64, \
                x=self.width//2, y=self.height//2)
        if self._state == STATE_PAUSED:
            self._text = GLabel(text="Press 'S' to Continue", font_size=64, \
            x=self.width//2, y=self.height//2)
        if self.input.is_key_pressed('s') and self._state == STATE_PAUSED:
            self._state = STATE_CONTINUE
            self._text = GLabel(text='')
            self._wave.restore_ship()
    ...
    def draw(self):
        ...
        if self._wave is None:
            ...
            if self._state == STATE_PAUSED or self._state == STATE_CONTINUE or \
            self._state == STATE_COMPLETE:
                self._text.draw(self.view)
